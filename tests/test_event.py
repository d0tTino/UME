...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "Missing required event fields: event_type",
        ),
        # Case 3: Missing 'timestamp'
        (
            {"event_type": "test", "payload": {}},
            "Missing required event fields: timestamp",
        ),
        # Case 4: Missing 'payload'
        (
            {"event_type": "test", "timestamp": 123},
            "Missing required event fields: payload",
        ),
        ...
        (
            {
                "event_type": "test_payload_type",
                "timestamp": int(time.time()),
                "payload": "not-a-dict",
            },
            "Invalid type for 'payload' in test_payload_type event (if provided): expected dict",
        ),
