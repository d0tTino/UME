...
The CLI automatically persists the graph state:
*   **On startup:** It attempts to load graph data from `~/.ume/ume_graph.json`. If the file exists and is valid, the CLI session starts with this data. Otherwise, it starts with an empty graph.
*   **On exit:** (using `exit`, `quit`, or Ctrl+D) The current graph state is automatically saved to `~/.ume/ume_graph.json`. This also occurs on `Ctrl+C` (KeyboardInterrupt).
*   The `snapshot_save` and `snapshot_load` commands can be used to work with custom snapshot files (or the default path if no argument is given), but the default auto-save on exit will always target `~/.ume/ume_graph.json`.
...
The Universal Memory Engine (UME) can be run as a RESTful service using FastAPI.

### Running the API Server

To start the API server locally (primarily for development):
1.  Ensure all dependencies are installed: `poetry install`. 
    *(Correction: uvicorn was added as a main dependency, so `poetry install` is sufficient).*
2.  Run Uvicorn:
    ```bash
...
The REST API service implements automatic persistence of the graph state:
*   **On server startup:** The API attempts to load graph data from a snapshot file. By default, this is `ume_graph.json` in the server's working directory. This path can be overridden by setting the `UME_SNAPSHOT_PATH` environment variable. If the file exists and is valid, the service starts with this data; otherwise, it starts with an empty graph.
*   **On graceful server shutdown:** (e.g., Ctrl+C when running Uvicorn, or SIGTERM) The current graph state is automatically saved to the same path (`API_SNAPSHOT_PATH` which is `UME_SNAPSHOT_PATH` or its default). Abrupt server termination (e.g., SIGKILL) may prevent this save.
*   The `/snapshot/save` and `/snapshot/load` endpoints can be used to manage snapshots to custom file paths during runtime, but the automatic persistence uses the configured `API_SNAPSHOT_PATH`.
...
*   **Description:** Submit an event to mutate the graph. The event is parsed and applied to the in-memory graph.
*   **Request Body (JSON):**
    ```json
    {
      ...
      "timestamp": 1678954321,
      "node_id": "node_alpha",
      "payload": {"name": "Alpha", "value": 100},
      "event_id": "optional_event_id_1",
      "source": "optional_source_app"
    }
    ```
    Or for an edge:
    ```json
    {
      "event_type": "CREATE_EDGE",
      "timestamp": 1678954322,
      "node_id": "node_alpha",         // Source node
      "target_node_id": "node_beta",
      "label": "RELATES_TO",
      "event_id": "optional_event_id_2"
      // payload is optional for edge events
    }
    ```
*   **Success Response (200 OK):**
    ...
    {
      "status": "ok",
      "event_id": "generated_or_provided_event_id"
    }
    ```
*   **Error Responses:**
    *   `422 Unprocessable Entity`: If event validation fails (e.g., missing required fields for the given `event_type`, incorrect data types). The detail will contain specific validation error messages from `parse_event`.
    *   `400 Bad Request`: If event processing fails (e.g., trying to create an already existing node, or create an edge to/from a non-existent node). The detail will contain messages from `apply_event_to_graph` or the graph adapter.
    *   `500 Internal Server Error`: For other unexpected server-side errors.

#### `GET /nodes`

*   **Description:** List all node identifiers currently present in the graph.
*   **Success Response (200 OK):**
    ```json
    {
...
*   **Description:** List all edges in the graph. Each edge is represented as a list: `[source_node_id, target_node_id, label]`.
*   **Success Response (200 OK):**
    ```json
    {
      "edges": [
        ["node_alpha", "node_beta", "RELATES_TO"],
        ["node_beta", "node_gamma", "HAS_PROPERTY"]
      ]
    }
    ```
...
*   **Description:** Returns all target node IDs connected from the given `node_id`. An optional `label` query parameter can be used to filter connections by that edge label.
*   **Path Parameter:** `node_id` (string, required)
*   **Query Parameter:** `label` (string, optional)
*   **Example URL:** `/neighbors/node_alpha?label=RELATES_TO`
*   **Success Response (200 OK):**
    ```json
    {
      "node_id": "node_alpha",
      "label_filter": "RELATES_TO", // or null if no label query param was used
      "neighbors": ["node_beta"]
    }
    ```
*   **Error Response:**
    *   `404 Not Found`: If the specified `node_id` does not exist in the graph.

#### `POST /snapshot/save`

*   **Description:** Writes the current graph state (nodes and edges) to a JSON file specified by the `path` in the request body.
*   **Request Body (JSON):**
    ```json
    {
      "path": "snapshots/my_graph_backup.json"
    }
    ```
*   **Success Response (200 OK):**
    ```json
    {
      "status": "success",
      "message": "Snapshot saved to snapshots/my_graph_backup.json"
    }
    ```
*   **Error Responses:**
    *   `400 Bad Request`: If the path is invalid or there's a permission error.
    *   `500 Internal Server Error`: For other snapshot save failures.

#### `POST /snapshot/load`

*   **Description:** Clears the current in-memory graph and restores its state from the JSON file specified by `path` in the request body.
*   **Request Body (JSON):**
    ```json
    {
      "path": "snapshots/my_graph_backup.json"
...
*   **Success Response (200 OK):**
    ...
    {
      "status": "success",
      "message": "Graph loaded from snapshots/my_graph_backup.json"
    }
    ```
*   **Error Responses:**
    *   `404 Not Found`: If the specified snapshot file does not exist.
    *   `400 Bad Request`: If the snapshot file is malformed (invalid JSON, incorrect data structure).
    *   `500 Internal Server Error`: For other snapshot load failures.

#### `POST /clear`

*   **Description:** Removes all nodes and edges from the in-memory graph, resetting it to an empty state.
*   **Success Response (200 OK):**
    ```json
    {
      "status": "success",
    ...
    ```
*   **Error Response:**
    *   `500 Internal Server Error`: If clearing the graph fails for an unexpected reason.
